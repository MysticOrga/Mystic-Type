...
### 1.1 Purpose
This document specifies the MysticType Protocol (MTP), a lightweight application-layer protocol designed for real-time multiplayer game communication over UDP.

### 1.2 Scope
MTP provides a simple, efficient packet structure for client-server and peer-to-peer game networking, with support for connection management, data transmission, and heartbeat mechanisms.
...
### 2.2 Transport Layer
MTP operates over UDP (User Datagram Protocol) on the transport layer. The protocol does not provide reliability guarantees; applications requiring reliable delivery must implement acknowledgment and retransmission mechanisms.

## 3. Packet Structure

### 3.1 General Format
All MTP packets consist of a fixed-size header followed by an optional variable-size payload:
...
**Payload Format:** Movement-specific data (0-255 bytes)

**Usage:** Player position updates, direction changes, velocity data, etc.
...
**Usage:** Sent by clients to update their position, broadcasted by server to other clients
...
### 6.1 Byte Order
All multi-byte fields MUST be transmitted in network byte order (big-endian). Implementations MUST use appropriate conversion functions (htons/ntohs).
...
The maximum packet size is 259 bytes (4-byte header + 255-byte payload). Implementations SHOULD ensure this fits within the network MTU to avoid fragmentation.
...
Implementations requiring security MUST implement these at a higher layer (e.g., DTLS over UDP).
...
Applications MAY define custom packet types in the reserved range for experimental features, but MUST NOT expect interoperability with other implementations.
